...
const orderId = 'TEST2-' + Date.now();
const monto = '340000';

// Probar con PUBLIC_KEY en vez de PRIVATE_KEY
const token = crypto.createHash('sha1').update(PUBLIC_KEY + orderId + monto).digest('hex');

console.log('='.repeat(80));
console.log('TEST PAGOPAR API - USANDO PUBLIC_KEY EN TOKEN');
console.log('='.repeat(80));
console.log('Order ID:', orderId);
console.log('Monto:', monto);
console.log('Token String:', PUBLIC_KEY + orderId + monto);
...
fetch('https://api.pagopar.com/api/comercios/2.0/iniciar-transaccion', {
  ...
  .then(data => {
    ...
    if (data.respuesta === false) {
      console.error('\n❌ PAGOPAR RECHAZÓ:', data.resultado);
    }

    if (data.respuesta === true || data.resultado?.data) {
      console.log('\n✅ ÉXITO!');
    }
  })
  .catch(error => {
