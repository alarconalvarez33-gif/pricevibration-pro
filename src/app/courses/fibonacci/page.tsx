...
export default async function FibonacciCoursePage() {
  ...
  if (!vip) {
    const paid = await prisma.productPurchase.findFirst({
      where: { userId: user.id, productId: 'fibonacci', status: 'paid' },
    })
    hasPurchased = !!paid

    if (!hasPurchased) {
      const pending = await prisma.productPurchase.findFirst({
        where: { userId: user.id, productId: 'fibonacci', status: 'pending' },
      ...
      <div className="border-b border-[#c9a227]/20 bg-[#0a0a0f]/95 backdrop-blur-sm">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-white">Curso de Fibonacci</h1>
              <p className="text-xs text-[#c9a227]">Descubrí el potencial oculto del Fibonacci — Curso Premium</p>
            </div>
          </div>
      ...
      {hasAccess ? (
        ...
        <div className="max-w-5xl mx-auto px-4 py-12">
          <div className="mb-8 text-center">
            <h2 className="text-3xl md:text-4xl font-bold mb-3">Curso de Fibonacci</h2>
            <p className="text-gray-400 text-lg">Descubrí el potencial del Fibonacci, por décadas oculto al público</p>
          </div>
          ...
          <div className="relative w-full rounded-2xl overflow-hidden border border-[#c9a227]/30 shadow-2xl shadow-[#c9a227]/10">
            <div style={{ padding: '56.25% 0 0 0', position: 'relative' }}>
              <iframe
                src="https://player.vimeo.com/video/1171349347?badge=0&autopause=0&player_id=0&app_id=58479"
        ...
        <div className="max-w-2xl mx-auto px-4 py-24 text-center">
          <PendingPaymentPoller productId="fibonacci" />
        ...
        <div className="max-w-2xl mx-auto px-4 py-24 text-center">
          ...
          <p className="text-gray-500 text-sm mb-8">
            <span className="text-[#c9a227] font-semibold">Curso de Fibonacci</span> — acceso único por{' '}
            <span className="text-white font-bold">499.000 GS</span>
          </p>
